...
class RDPG(object):
    ...
    def train(self, num_iterations, checkpoint_path, debug):
        ...
        while step < num_iterations:
            ...
            x = r * math.cos(deg)
            y = r * math.sin(deg)
            self.env.set_goal([x, y])
            prRed('[New Goal] Step_{:07d}: ({},{})'.format(step, x, y))
            episode_steps = 0
...
#        self.actor_optim.step()
    ...
    def test(self, num_episodes, model_path, visualize=True, debug=False):
        if self.agent.load_model(model_path) == False:
    ...
    def load_model(self, checkpoint_path):
        ...
        if os.path.isfile(checkpoint_path):
            ...
            hard_update(self.agent.actor_target, self.agent.actor) # Make sure target is with the same weight
    ...
    def save_model(self, output, step):
        self.save_checkpoint({
                'epoch': step + 1,
                'actor_state_dict': self.agebt.actor.state_dict(),
